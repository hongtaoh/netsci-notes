...
For example, in the above network, nodes 1 and 2have a degree of two. Node 3 has a degree of three and Node 4 has a degree of 1. 

For the $i^{th}$ node in a network, we'll denote its degree as $k_i$. Therefore, for the above network, $k_4=1$, $k_1 = k_2 = 2$, $k_3$=3. 
...
**Average degree**, denoted as $\langle k \rangle$ is simply the mean of all the node degrees in a network. For the network above (Figure \@ref(fig:nunit2-nodes)), $\langle k \rangle = \frac{1}{4} \cdot (k_1 + k_2 + k_3 + k_4) = \frac{1}{4} \cdot (2+2+3+1) = 2$. This means that on average, each node in the network has 2.25 links. 
